...
- Structured (schema-validated) generation for callers that must never receive
  free-form text they can't trust (see generate_structured)
"""

import time
import logging
from typing import Any, Awaitable, Callable, List, Optional, Tuple, Dict, Type, TypeVar
...
class LLMGateway:
    ...
    async def _run_with_fallback(
        self,
        invoke: Callable[[ProviderDeployment], Awaitable[Any]],
        log_label: str = "",
    ) -> Tuple[Any, str, str, List[ProviderStatusEvent]]:
        """
        Shared ordered-fallback loop: tries each available deployment in turn, classifying
        errors into retryable (cooldown / disable-and-continue) vs non-retryable (raise).

        `invoke(deployment)` performs the actual provider call and returns whatever the
        caller wants back (a raw AIMessage, a validated Pydantic instance, ...) -- this
        method only owns the deployment selection, retry, and status-event bookkeeping
        shared by every LLM call shape (see generate() and generate_structured()).

        Returns:
            (result, provider_name, model_name, status_events)
        """
        ...
        for deployment in available:
            ...
            try:
                logger.info(
                    f"🌐 {log_label}Connecting to Cloud LLM Provider: {deployment.name} ({deployment.default_model}) "
                    f"[Attempt {attempts}/{self.max_attempts}]..."
                )
                start_time = time.time()
                result = await invoke(deployment)
                ...
                logger.info(f"✅ {log_label}LLM Call Succeeded via {deployment.name} in {duration_ms}ms")
                return result, deployment.name, deployment.default_model, status_events
    ...
    async def generate(
        ...
        """
        Executes a free-form chat completion with automatic fallback across providers.

        Returns:
            (reply, provider_name, model_name, usage_dict, status_events)
        """

        async def _invoke(deployment: ProviderDeployment):
            llm = ChatOpenAI(
        ...
        ai_response, provider_name, model_name, status_events = await self._run_with_fallback(_invoke)
        ...
        return str(reply_content), provider_name, model_name, usage, status_events
    ...
    async def generate_structured(
    ...
    ) -> Tuple[SchemaT, str, str, List[ProviderStatusEvent]]:
        """
        Same fallback/cooldown/error-classification behavior as generate(), but constrains
        the model's output to a validated instance of `schema` instead of raw text.

        Used wherever a caller must guarantee a strictly-typed response it can trust
        structurally (e.g. AI narration that may only reference entity ids that already
        exist in the caller's input -- see services/summary_service.py). Uses LangChain's
        tool-calling structured output mode (`method="function_calling"`) rather than
        OpenAI's native strict JSON-schema mode, since the latter is not portable across
        the OpenAI-compatible endpoints this gateway fans out to (Groq, Mistral, Cerebras,
        Gemini's OpenAI-compat surface, ...).

        Returns:
            (parsed_schema_instance, provider_name, model_name, usage_dict, status_events)
        """
        ...
        async def _invoke(deployment: ProviderDeployment):
            ...
            # include_raw=True keeps the underlying AIMessage (for token usage) alongside
            # the parsed object, and surfaces schema-parsing failures as `parsing_error`
            # instead of raising -- we re-raise it ourselves so the shared fallback loop
            # treats a malformed structured response the same as any other provider error
            # and simply tries the next deployment.
            structured_llm = llm.with_structured_output(
                schema, method="function_calling", include_raw=True
            )
            outcome = await structured_llm.ainvoke(messages)
            if outcome.get("parsing_error") is not None:
                raise ValueError(f"structured_output_parse_error: {outcome['parsing_error']}")
            return outcome

        outcome, provider_name, model_name, status_events = await self._run_with_fallback(
            _invoke, log_label="[structured] "
        )
        ...
        return outcome["parsed"], provider_name, model_name, usage, status_events
